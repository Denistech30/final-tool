...
function App() {
  ...
  const [selectedSequence, setSelectedSequence] = useState<"firstSequence" | "secondSequence" | "thirdSequence" | "fourthSequence" | "fifthSequence" | "sixthSequence">("firstSequence");
  ...
  const handleGenerateStudentReport = (studentName: string) => {
    const doc = new jsPDF();
    ...
    doc.setFontSize(16);
    doc.text(studentName, pageWidth / 2, 30, { align: "center" });
    
    let yPosition = 50;

    const sequences = ["First", "Second", "Third", "Fourth", "Fifth", "Sixth"];
    const sequenceKeys = [
      "firstSequence",
      "secondSequence",
      "thirdSequence",
    ...
    sequenceKeys.forEach((seq, index) => {
      const sequenceMarks = marks.find(
        (m, idx) => students[idx] === studentName
      )?.[seq as keyof StudentMarks];

      if (sequenceMarks && Object.keys(sequenceMarks).length > 0) {
        doc.setFontSize(14);
        doc.text(`${sequences[index]} Sequence Marks`, 20, yPosition);
        
        const tableData = subjects.map((subject) => [
          subject.name,
          sequenceMarks[subject.name]?.toString() || "-",
          subject.total.toString(),
        ]);

        autoTable(doc, {
          startY: yPosition + 5,
          head: [["Subject", "Mark", "Total"]],
          body: tableData,
          theme: "grid",
        });
    ...
    const termResults = [
      { title: "First Term", results: firstTermResults },
      { title: "Second Term", results: secondTermResults },
      { title: "Third Term", results: thirdTermResults },
    ];

    termResults.forEach(({ title, results }) => {
      const studentResult = results.find((r) => r.student === studentName);
      if (studentResult) {
        doc.setFontSize(14);
        doc.text(`${title} Summary`, 20, yPosition);
        
        autoTable(doc, {
          startY: yPosition + 5,
          head: [["Average", "Rank", "Class Average"]],
          body: [[
            `${studentResult.average.toFixed(2)}/20`,
            studentResult.rank.toString(),
            `${(title === "First Term" ? firstTermClassAverage : 
              title === "Second Term" ? secondTermClassAverage : 
              thirdTermClassAverage)?.toFixed(2)}/20`,
          ]],
          theme: "grid",
        });
    ...
    const annualResult = annualResults.find((r) => r.student === studentName);
    if (annualResult) {
      doc.setFontSize(14);
      ...
      autoTable(doc, {
        startY: yPosition + 5,
        head: [["Final Average", "Rank", "Class Average"]],
        body: [[
          `${annualResult.finalAverage.toFixed(2)}/20`,
          annualResult.rank.toString(),
          `${annualClassAverage?.toFixed(2)}/20`,
        ]],
        theme: "grid",
      });
    ...
    const comment = studentComments[studentName];
    if (comment) {
      doc.setFontSize(14);
      doc.text("Teacher's Comments", 20, yPosition);
      
      const splitComment = doc.splitTextToSize(comment, pageWidth - 40);
      doc.setFontSize(12);
      doc.text(splitComment, 20, yPosition + 10);
    }

    doc.save(`${studentName.replace(/\s+/g, "_")}_report.pdf`);
  };

  const handleOpenCommentDialog = (studentName: string) => {
  ...
  const handleSaveComment = () => {
    if (selectedStudent) {
      setStudentComments({
        ...studentComments,
        [selectedStudent]: studentComments[selectedStudent] || "",
      });
    }
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12}>
          <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, justifyContent: { xs: "center", sm: "flex-start" } }}>
            <Button variant="contained" color="primary" onClick={() => setStudentsOpen(true)} sx={{ minWidth: { xs: 100, sm: 120 } }}>
              Students
            </Button>
            <Button variant="contained" color="secondary" onClick={() => setSubjectsOpen(true)} disabled={students.length === 0} sx={{ minWidth: { xs: 100, sm: 120 } }}>
              Subjects
            </Button>
            <Button variant="outlined" color="error" onClick={handleResetData} sx={{ minWidth: { xs: 100, sm: 120 } }}>
              Reset Data
            </Button>
          </Box>
        </Grid>
        ...
        {subjects.length > 0 && students.length > 0 && (
          <Grid item xs={12}>
            <Paper elevation={3} sx={{ p: { xs: 1, sm: 2 }, borderRadius: 2, mt: 2 }}>
              ...
              <FormControl sx={{ mb: 2, minWidth: 200 }}>
                ...
                <Select
                  ...
                  label="Sequence"
                  onChange={(e) => setSelectedSequence(e.target.value as "firstSequence" | "secondSequence" | "thirdSequence" | "fourthSequence" | "fifthSequence" | "sixthSequence")}
                  size="small"
                >
              ...
              <Box sx={{ overflowX: "auto" }}>
                <Table sx={{ bgcolor: "white", borderCollapse: "separate", borderSpacing: 0, "& th, & td": { borderBottom: "1px solid #e0e0e0", padding: { xs: "4px", sm: "8px" }, fontSize: { xs: "0.75rem", sm: "0.875rem" } } }}>
                  <TableHead>
                    <TableRow>
                      <TableCell sx={{ fontWeight: "bold", minWidth: { xs: 100, sm: 150 }, position: "sticky", left: 0, bgcolor: "white", zIndex: 1 }}>
                        Student
                      </TableCell>
                      {subjects.map((subject) => (
                        <TableCell key={subject.name} sx={{ fontWeight: "bold", minWidth: { xs: 80, sm: 120 } }}>
                  ...
                  <TableBody>
                    {students.map((student, studentIndex) => (
                      <TableRow key={student}>
                        <TableCell sx={{ verticalAlign: "middle", position: "sticky", left: 0, bgcolor: "white", zIndex: 1 }}>
                          {student}
                        </TableCell>
                        {subjects.map((subject) => {
                          ...
                          return (
                            <TableCell key={subject.name}>
                              <TextField
                                ...
                                size="small"
                                value={mark}
                                onChange={(e) => handleMarkChange(studentIndex, subject.name, e.target.value, subject.total)}
                                inputProps={{ min: 0, max: subject.total, step: "0.01" }}
                                sx={{
                                  ...
                                  "& .MuiOutlinedInput-root": {
                                    ...
                                    "&.Mui-focused fieldset": {
                                      borderColor: isBelowAverage ? "red" : "primary.main",
                                    },
                                  },
                                }}
                              />
              ...
              <Box sx={{ display: "flex", gap: 2, mt: 2 }}>
                <Button variant="contained" color="primary" onClick={calculateSequenceResults} disabled={!hasMarks} sx={{ minWidth: { xs: 100, sm: 120 } }}>
                  Calculate Results
                </Button>
                <Button variant="contained" color="secondary" onClick={handleCalculateTermResults} disabled={!hasMarks} sx={{ minWidth: { xs: 100, sm: 120 } }}>
                  Term Results
                </Button>
              </Box>
            </Paper>
        ...
        {(sequenceResults.length > 0 || firstTermResults.length > 0 || secondTermResults.length > 0 || thirdTermResults.length > 0 || annualResults.length > 0) && (
          <Grid item xs={12}>
            <Paper elevation={3} sx={{ p: { xs: 1, sm: 2 }, borderRadius: 2, mt: 2 }}>
              <Typography variant="h6" gutterBottom sx={{ fontSize: { xs: "1rem", sm: "1.25rem" } }}>
                Results
              ...
              <FormControl sx={{ mb: 2, minWidth: 200 }}>
                <InputLabel id="result-view-select-label">Result View</InputLabel>
                <Select
                  labelId="result-view-select-label"
                  value={selectedResultView}
                  label="Result View"
                  onChange={(e) => setSelectedResultView(e.target.value as "sequence" | "firstTerm" | "secondTerm" | "thirdTerm" | "annual")}
                  size="small"
                >
              ...
              <Box sx={{ overflowX: "auto" }}>
                <Table sx={{ bgcolor: "white", borderCollapse: "separate", borderSpacing: 0, "& th, & td": { borderBottom: "1px solid #e0e0e0", padding: { xs: "4px", sm: "8px" }, fontSize: { xs: "0.75rem", sm: "0.875rem" } } }}>
                  <TableHead>
                    <TableRow>
                      <TableCell sx={{ fontWeight: "bold", minWidth: { xs: 40, sm: 60 } }}>
                        Rank
                      </TableCell>
                      <TableCell sx={{ fontWeight: "bold", minWidth: { xs: 100, sm: 150 }, position: "sticky", left: 0, bgcolor: "white", zIndex: 1 }}>
                        Student
                      ...
                      {selectedResultView === "annual" ? (
                        <>
                          <TableCell sx={{ fontWeight: "bold", minWidth: { xs: 80, sm: 120 } }}>
                            First Term Avg
                          </TableCell>
                          <TableCell sx={{ fontWeight: "bold", minWidth: { xs: 80, sm: 120 } }}>
                            Second Term Avg
                  ...
                  <TableBody>
                    {selectedResultView === "sequence" && sequenceResults.map((result) => (
                      <TableRow key={result.student}>
                        ...
                        <TableCell sx={{ color: result.average < PASSING_MARK ? "red" : "inherit" }}>
                          {result.average.toFixed(2)}
                        </TableCell>
                      </TableRow>
                    ))}
                    {selectedResultView === "firstTerm" && firstTermResults.map((result) => (
                      <TableRow key={result.student}>
                        <TableCell>{result.rank}</TableCell>
                        <TableCell sx={{ position: "sticky", left: 0, bgcolor: "white", zIndex: 1 }}>
                          {result.student}
                    ...
                    {selectedResultView === "secondTerm" && secondTermResults.map((result) => (
                      <TableRow key={result.student}>
                        <TableCell>{result.rank}</TableCell>
                        <TableCell sx={{ position: "sticky", left: 0, bgcolor: "white", zIndex: 1 }}>
                          {result.student}
                    ...
                    {selectedResultView === "thirdTerm" && thirdTermResults.map((result) => (
                      <TableRow key={result.student}>
                        <TableCell>{result.rank}</TableCell>
                        <TableCell sx={{ position: "sticky", left: 0, bgcolor: "white", zIndex: 1 }}>
                          {result.student}
                    ...
                    {selectedResultView === "annual" && annualResults.map((result) => (
                      <TableRow key={result.student}>
                        <TableCell>{result.rank}</TableCell>
                        <TableCell sx={{ position: "sticky", left: 0, bgcolor: "white", zIndex: 1 }}>
                          {result.student}
                        ...
                        <TableCell sx={{ color: result.finalAverage < PASSING_MARK ? "red" : "inherit" }}>
                          {result.finalAverage.toFixed(2)}
                        </TableCell>
                      </TableRow>
                    ))}
              ...
              <Typography variant="body1" sx={{ mt: 2, fontSize: { xs: "0.75rem", sm: "1rem" } }}>
                Class Average:{" "}
              ...
              <Button variant="contained" color="primary" onClick={handleDownloadPDF} sx={{ mt: 2, minWidth: { xs: 100, sm: 120 } }}>
                Download PDF
              </Button>
            </Paper>
          </Grid>
                    ...
                    <TableRow key={student}>
                      ...
                      <TableCell>
                        <Box sx={{ display: "flex", gap: 1 }}>
                          <Button variant="outlined" size="small" onClick={() => handleOpenCommentDialog(student)}>
                            Add Comment
                          </Button>
                          <Button variant="contained" size="small" onClick={() => handleGenerateStudentReport(student)} startIcon={<PersonIcon />}>
                            Generate Report
                          </Button>
                        </Box>
                      </TableCell>
        ...
        <Modal open={studentsOpen} onClose={() => setStudentsOpen(false)}>
          <Box sx={modalStyle}>
            ...
            <Table sx={{ "& th, & td": { padding: { xs: "4px", sm: "8px" }, fontSize: { xs: "0.75rem", sm: "0.875rem" } } }}>
              <TableHead>
                <TableRow>
              ...
              <TableBody>
                ...
                <TableRow>
                  ...
                  <TableCell>
                    <IconButton onClick={handleAddStudent} disabled={newStudentName.trim() === ""}>
            ...
            <Table sx={{ "& th, & td": { padding: { xs: "4px", sm: "8px" }, fontSize: { xs: "0.75rem", sm: "0.875rem" } } }}>
              ...
              <TableBody>
                    ...
                    <TableCell>
                      {editSubjectIndex === index ? (
                        <TextField
                          ...
                          onChange={(e) => setEditSubjectValue({ ...editSubjectValue, name: e.target.value })}
                          size="small"
                          autoFocus
                    ...
                    <TableCell>
                      {editSubjectIndex === index ? (
                        <TextField
                          ...
                          onChange={(e) => setEditSubjectValue({ ...editSubjectValue, total: parseInt(e.target.value, 10) || 0 })}
                          size="small"
                          inputProps={{ min: 0 }}
                ...
                <TableRow>
                  ...
                  <TableCell>
                    <IconButton onClick={handleAddSubject} disabled={newSubjectName.trim() === "" || newSubjectTotal.trim() === "" || parseInt(newSubjectTotal, 10) <= 0}>
                      <AddIcon color="primary" fontSize="small" />
                    </IconButton>
                  </TableCell>
                </TableRow>
        ...
        <Dialog open={commentDialogOpen} onClose={() => setCommentDialogOpen(false)} maxWidth="sm" fullWidth>
          <DialogTitle>Add Comment for {selectedStudent}</DialogTitle>
          <DialogContent>
            <TextField
              autoFocus
              margin="dense"
              ...
              onChange={(e) => setStudentComments({ ...studentComments, [selectedStudent]: e.target.value })}
            />
          </DialogContent>
          <DialogActions>
